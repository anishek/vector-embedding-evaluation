...
def e5_large_embedding(input_data, read_from_location):
    model = SentenceTransformer("intfloat/multilingual-e5-large-instruct")
    # to get the max token length that the model can process used
    print("Max:", model.max_seq_length)  # Output: 512
    embeddings_to_add = []
    for text in recursive_chunking(input_data, model.max_seq_length):
...
def main():
    ...
    e5_large_embedding(content, read_from_location)
